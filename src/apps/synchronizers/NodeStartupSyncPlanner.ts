...
import { NodeStartupSyncPlan } from './NodeStartupSyncPlan';
import NodeStartupSyncPolicy from './NodeStartupSyncPolicy';

export default class NodeStartupSyncPlanner {
  ...
  private async communityRequests(
    ...
    const communities = await this.communityRepository.findSyncable();
  ...
  public async plan(connectedPeerCount: number): Promise<NodeStartupSyncPlan> {
