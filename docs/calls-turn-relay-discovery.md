...
| `CALLS_ICE_TRANSPORT_POLICY` | Defaults to `relay` when `/calls/ice-servers` can return a usable TURN server. Without TURN URLs plus valid credentials, the endpoint returns `all` so clients can still use direct ICE candidates. |
