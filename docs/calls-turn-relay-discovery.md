...
The signature proves that the libp2p peer that owns `peerId` announced the
record. It does not prove that the TURN service is reachable; WebRTC ICE still
does the final connectivity check.
...
- `version` is `1`.
- `role` is `call-relay`.
- `peerId`, `publicKey` and `signature` are strings.
...
flowchart TD
    ...
    Expiry -- yes --> Signature{"Signature valid for peerId?"}
    Signature -- no --> Ignore4["Ignore"]
...
| Record signature invalid | Record is ignored. |
