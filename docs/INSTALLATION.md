...
| `PIGEON_PUBLIC_BOOTSTRAP_ENABLED` | `true` | No | Enables explicit public libp2p bootstrap peers for the public Helia/IPFS and standalone GossipSub runtimes. |
| `PIGEON_PUBLIC_BOOTSTRAP_MULTIADDRS` | libp2p DNSADDR bootstrap peers with `/p2p/` ids | No | Comma-separated public bootstrap multiaddrs used so nodes can join the public routing/DHT and relay-announcement layers before discovering private relay records. If overridden, every address must include the peer id (`/p2p/<peerId>` or compatible alias); a bare `/dnsaddr/bootstrap.libp2p.io` entry is ignored by `@libp2p/bootstrap`. |
