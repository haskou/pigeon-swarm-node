...
describe('OrbitDBDomainEventProjector', () => {
  ...
  it('projects community events into replicated read model stores', async () => {
    ...
    expect(stores.heads.put).toHaveBeenCalledWith(
      'community-channel-message-index:community-1:channel-1',
      expect.objectContaining({
        channelId: 'channel-1',
        communityId: 'community-1',
        messages: expect.arrayContaining([
          expect.objectContaining({ id: 'community-message-1' }),
        ]),
      }),
    );
    expect(stores.reactions.put).toHaveBeenCalledWith(
  ...
  it('projects community channel events into community heads and member indexes', async () => {
    const stores = fakeStores();
    ...
    await projector.project(
      storesFrom(stores),
      replicatedMessage(
        CommunityChannelWasCreatedEvent.EVENT_NAME,
        {
          channel: {
    ...
    await projector.project(
      storesFrom(stores),
      replicatedMessage(
        CommunityChannelWasRenamedEvent.EVENT_NAME,
        {
          channelId: 'channel-1',
          communityId: 'community-1',
          memberIds: ['identity-1'],
        ...
        communities: expect.arrayContaining([
          expect.objectContaining({
            ...
            textChannels: expect.arrayContaining([
              expect.objectContaining({ id: 'channel-1', name: 'updates' }),
            ]),
          }),
        ]),
      }),
    ...
    await projector.project(
      storesFrom(stores),
      replicatedMessage(
        CommunityChannelWasDeletedEvent.EVENT_NAME,
        {
          channelId: 'channel-1',
          communityId: 'community-1',
          memberIds: ['identity-1'],
  ...
  it('projects community member leave events into community heads and member indexes', async () => {
    const stores = fakeStores();
      ...
      replicatedMessage(
        CommunityWasCreatedEvent.EVENT_NAME,
        {
          community: {
            ...
            description: 'community',
            id: 'community-1',
            memberIds: ['identity-1', 'identity-2'],
            name: 'community',
            networkId: 'network-1',
            ownerIdentityId: 'identity-1',
            textChannels: [],
    ...
    expect(stores.heads.put).toHaveBeenCalledWith(
      'community-member-index:identity-2',
      expect.objectContaining({
        communities: [],
      }),
    );
