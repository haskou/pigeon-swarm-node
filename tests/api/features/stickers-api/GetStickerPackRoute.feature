Feature: Get sticker pack route
  As an API consumer
  I want to read a sticker pack
  So that clients can render its current stickers

  Background:
    Given I register a test IPFS network "api-stickers-network"

  Scenario: Read a sticker pack by id
    ...
    And I sign the current sticker pack request
