Feature: Get community discovery API
  ...
  Scenario: Discover communities by search text
    ...
    And I register an in-memory IPFS network "communities-api-discovery-network"
  ...
  Scenario: Hide communities from discovery
    Given I am an anonymous user
    And I register an in-memory IPFS network "communities-api-hidden-discovery-network"
