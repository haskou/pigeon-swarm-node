Feature: Post notification route
  ...
  Scenario: Create a group conversation invitation notification
    Given I am an anonymous user
    And I set a group conversation invitation notification body
    And I sign the current notification creation request
    When I POST to "/notifications/"
    Then response code is equal to 200
    And response contains a valid resource with the following fields
      ...
      | status | unread                        |
      | state  | pending                       |
    And response body should contain "encrypted-group-conversation-key"
